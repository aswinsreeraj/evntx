          ...
          {loading ? (
             ...
             <div key={event.ID} className="bg-white border border-gray-100 rounded-[24px] p-4 flex gap-6 hover:shadow-sm transition-shadow">
                <div className="w-[300px] shrink-0 aspect-[16/9] rounded-xl overflow-hidden bg-gray-100 relative">
                   {event.CoverImageURL ? (
                      <img src={`${import.meta.env.VITE_API_BASE_URL}${event.CoverImageURL}`} alt={event.Title} className="w-full h-full object-cover" />
                   ) : (
                ...
                <div className="flex-1 flex flex-col justify-center py-2">
                   <div>
                      ...
                      <p className="text-[13px] text-gray-500 flex items-center gap-1.5 mt-2"><MapPin className="w-3.5 h-3.5"/> {event.VenueName}, {event.City}</p>
                   </div>
