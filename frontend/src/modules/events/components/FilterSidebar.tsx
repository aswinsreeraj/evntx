...
export default function FilterSidebar({
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (showCatDropdown && !target.closest('.cat-dropdown-container')) {
    ...
    <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col gap-6 w-full max-w-sm">
      ...
      <div>
        ...
        <div className="bg-red-50 rounded-xl px-4 pb-3 pt-6 flex items-center gap-3 relative">
           <Calendar className="w-5 h-5 text-red-500 hidden sm:block mt-1" />
           <div className="flex gap-2 items-center flex-1 w-full relative">
              <div className="flex flex-col flex-1 relative">
                <span className="text-[10px] uppercase font-bold text-red-400 absolute -top-4.5 left-0">Start Date</span>
                <input
                   type="date"
                   className="bg-transparent text-sm text-gray-700 outline-none w-full !p-0"
              ...
              <div className="flex flex-col flex-1 relative">
                <span className="text-[10px] uppercase font-bold text-red-400 absolute -top-4.5 left-0">End Date</span>
                <input
                   type="date"
                   className="bg-transparent text-sm text-gray-700 outline-none w-full !p-0"
                   value={localEndDate}
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter' && locationInput.trim()) {
                  e.preventDefault();
                  if (!localCities.includes(locationInput.trim())) {
                    setLocalCities([...localCities, locationInput.trim()]);
                  }
                  setLocationInput("");
                }
              }}
              className="w-full bg-red-50 text-gray-700 text-sm px-4 py-3 rounded-xl outline-none focus:ring-1 focus:ring-red-200"
            />
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {localCities.map(city => (
              <span key={city} className="inline-flex items-center gap-1.5 bg-gray-100 text-gray-700 px-3 py-1.5 rounded-full text-sm">
                <X
                  className="w-4 h-4 text-red-500 cursor-pointer hover:text-red-700"
                  onClick={() => setLocalCities(localCities.filter(c => c !== city))}
                />
                {city}
              </span>
            ))}
