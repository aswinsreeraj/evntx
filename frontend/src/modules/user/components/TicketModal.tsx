...
function PrintTicket({ booking, ticket, attendeeName }: { booking: BookingRecord; ticket: TicketRecord; attendeeName: string }) {
  ...
  return (
    <div className="ticket-print-page">
      <div style={{ maxWidth: 600, margin: "0 auto", borderRadius: 24, overflow: "hidden", boxShadow: "0 8px 40px rgba(0,0,0,0.18)", background: "#111821", color: "#fff" }}>
        {coverSrc ? (
          <img src={coverSrc} alt={booking.event_title} style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />
        ) : (
          <div style={{ width: "100%", height: 200, background: "linear-gradient(135deg,#4b5563,#1f2937)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ fontSize: 64, fontWeight: 700, color: "rgba(255,255,255,0.2)" }}>{(booking.event_title || "E")[0].toUpperCase()}</span>
          </div>
        )}

        <div style={{ padding: "24px 32px 20px" }}>
          <h2 style={{ margin: 0, fontSize: 22, fontWeight: 700 }}>{booking.event_title}</h2>
          <p style={{ margin: "6px 0 0", fontSize: 14, color: "#a0aec0" }}>{booking.venue || booking.event_city}</p>
        </div>

        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px 32px", padding: "0 32px 24px", color: "#fff" }}>
          <div>
            <p style={{ margin: 0, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#8d949e" }}>Date</p>
            <p style={{ margin: "4px 0 0", fontSize: 17 }}>{formatDateBadge(booking.event_start_time)}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#8d949e" }}>Time</p>
            <p style={{ margin: "4px 0 0", fontSize: 17 }}>{formatEventTime(booking.event_start_time)}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#8d949e" }}>Ticket Type</p>
            <p style={{ margin: "4px 0 0", fontSize: 17, fontWeight: 600 }}>{ticket.ticket_type ?? "General"}</p>
          </div>
          <div>
            <p style={{ margin: 0, fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", color: "#8d949e" }}>Ticket ID</p>
            <p style={{ margin: "4px 0 0", fontSize: 17 }}>{ticket.ticket_code ?? booking.booking_id.slice(0, 4)}</p>
          </div>
        </div>

        <div style={{ borderTop: "2px dashed rgba(255,255,255,0.2)", margin: "0 32px", position: "relative" }}>
          <div style={{ position: "absolute", left: -44, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "#fff" }} />
          <div style={{ position: "absolute", right: -44, top: "50%", transform: "translateY(-50%)", width: 32, height: 32, borderRadius: "50%", background: "#fff" }} />
        </div>

        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", padding: "20px 32px 28px" }}>
          <div>
            <p style={{ margin: 0, fontSize: 17, fontWeight: 600, color: "#fff" }}>{attendeeName}</p>
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#7b838e" }}>Booking ID: {booking.booking_id}</p>
          </div>
          <TicketQr value={`${booking.booking_id}-${ticket.ticket_id ?? "ticket"}`} />
        </div>
      </div>
    </div>
  )
...
export default function TicketModal({ booking, tickets, open, onClose }: Props) {
  ...
  const printPortal = createPortal(
    <div id="tickets-print-root">
      {tickets.map((t) => (
        <PrintTicket key={t.ticket_id} booking={booking} ticket={t} attendeeName={attendeeName} />
      ))}
    </div>,
    document.body
  )
